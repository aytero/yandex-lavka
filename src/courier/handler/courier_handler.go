...
func (h *CourierHandler) GetCouriers(ctx echo.Context) error {
	...
	couriers, err := h.uc.GetCouriers(ctx.Request().Context(), int32(limit), int32(offset))
	couriersResponse := dtoCourier.ParseFromEntitySlice(couriers)
	if err != nil {
		log.Infof("CourierHandler - GetCouriers: %w", err)
		return ctx.JSON(http.StatusBadRequest, dtoCourier.BadRequestResponse{})
	}
	return ctx.JSON(http.StatusOK, couriersResponse)
}
...
func (h *CourierHandler) CreateCourier(ctx echo.Context) error {
	...
	couriers, err := h.uc.CreateCourier(ctx.Request().Context(), req.MapToModel())
	if err != nil {
		log.Infof("CourierHandler - CreateCouriers: %w", err)
		return ctx.JSON(http.StatusBadRequest, dtoCourier.BadRequestResponse{})
	}
	createResponse := dtoCourier.ParseFromEntitySlice(couriers)
...
func (h *CourierHandler) GetCourier(ctx echo.Context) error {
	...
	courierResponse := dtoCourier.ParseFromEntity(entry)
	return ctx.JSON(http.StatusOK, courierResponse)
}
...
func (h *CourierHandler) GetCourierMetaInfo(ctx echo.Context) error {
	...
	cour, err := h.uc.GetCourierMetaInfo(ctx.Request().Context(), courierId, startDate, endDate)
	metaResponse := dtoCourier.ParseFromEntityMeta(cour)
