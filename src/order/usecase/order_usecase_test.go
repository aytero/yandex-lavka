...
import (
	...
	mockOrder "yandex-team.ru/bstask/order/repository/mocks"
)
...
func TestGetOrders(t *testing.T) {
	...
	repo := mockOrder.NewMockRepository(ctl)
	...
	repo.EXPECT().GetOrders(ctx, int32(1), int32(0)).Return(mockRepo, nil).Times(1)

	service := NewOrderUsecase(repo)
	...
	repo.EXPECT().GetOrders(ctx, int32(1), int32(0)).Return(nil, errDb).Times(1)
